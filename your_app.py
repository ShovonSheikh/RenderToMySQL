                        ...
                        for member in tar.getmembers():
                            if "libaio.so.1" in member.name:
                                ...
                                if extracted.exists():
                                    ...
                                    target.chmod(0o755)
                                    print(f"Installed libaio to: {target}")
                                    return target
            except Exception:
                pass
    except Exception as e:
...
[ -n "$LD_PRELOAD" ] || export LD_PRELOAD="{str(LIB_DIR)}/libaio.so.1"
